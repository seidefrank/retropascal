...
#include "winrt/compat.h"   // some mappings to make it compile more easily under WinRT
...
{
  int           UseCount;           // ref count
  word          OldKp;              // code stack pointer when segment was loaded, pop to this to unload
  word          Seg;                // proc table
  word          SegBase;            // load address of segment
} SegDict_t;
...
// BEGIN saved on suspend

SegDict_t       SegDict[SEG_DICT_SIZE];

/* Official P-Machine registers */
word    Sp;     // stack pointer, for TOS arithmetic and function args but not for function stack frames, i.e. should never overflow, really
word    Ipc;    // instruction pointer, relative to IpcBase
word    IpcBase;// code start of the current function (set for each function call)
word    Seg;    // pointer to current segment (char segid, char #procs), Seg[-n] = self-rel ptr JTab entry
word    JTab;   // pointer to current procedure descriptor (char procid, char lexlevel), [-1]=base (self-rel), [-2]=Exit code (self-rel), [-3]=#param bytes, [-4]=#local-var bytes
word    Kp;     // code stack --grows from top to bottom
word    Mp;     // function stack frames --interleaves with code stack
word    Np;     // 'new' stack --New() allocates from Np and increases it by size
word    Base;   // (seems mirrored into SYSCOM->STKBASE)

word    Syscom; // address of SYSCOM record

/* Additional Bookkeeping */
/*static*/ word CurrentIpc;     // written by Processor() on each instr., read by Debugger() and XeqError()
/*static*/ word BaseMp;         // Mp right after load_system, never modified, read by XeqError() and CBP instruction
unsigned int    Level=0;    // proc nesting level, for debugging only

// END saved on suspend

unsigned int    TraceLevel=-1;
jmp_buf         ProcessNextInstrunction;

// suspend flag
static volatile char Irq = 0;
...
typedef struct
{
    word Sp, Ipc, IpcBase, Seg, JTab, Kp, Mp, Np, Base;
    word Syscom;
    word CurrentIpc, BaseMp;
    unsigned int Level;
} CPUState;
...
{
    CPUState s = { Sp, Ipc, IpcBase, Seg, JTab, Kp, Mp, Np, Base, Syscom, CurrentIpc, BaseMp, Level };
    size_t reqsize = sizeof(s) + sizeof(SegDict);
    if (reqsize > bufsize)
        return -1;
    memcpy(buf, &s, sizeof(s));
    memcpy((char *) buf + sizeof(s), &SegDict, sizeof(SegDict));
    return (int) reqsize;
}

int RestoreCPU(const void * buf, size_t savedsize)
{
    CPUState * s = (CPUState *) buf;
    size_t reqsize = sizeof(*s) + sizeof(SegDict);
    if (reqsize != savedsize)
        return -1;
    Sp = s->Sp; Ipc = s->Ipc; IpcBase = s->IpcBase; Seg = s->Seg; JTab = s->JTab; Kp = s->Kp; Mp = s->Mp; Np = s->Np; Base = s->Base;
    Syscom = s->Syscom;
    CurrentIpc = s->CurrentIpc; BaseMp = s->BaseMp;
    Level = s->Level;
    memcpy(&SegDict, (char *) buf + sizeof(*s), sizeof(SegDict));
    return 0;
}
...
warning(const char *Msg, ...)
{
    ...
    char Buffer[512];
    va_start(ap, Msg);
    vsnprintf_s(Buffer, sizeof(Buffer)/sizeof(*Buffer), sizeof(Buffer), Msg, ap);
    va_end(ap);
...
DumpCore(void)
{
    FILE *f;
    char filename[100];
    _snprintf(filename, sizeof(filename), "%s.core", progname_get());
    f = fopen(filename, "w");
    if (f)
    {
        MemDump(f, 0, 0xffff);
...
CspLoadSegment(byte SegNo)
{
    ...
    {
        word SegUnit = MemRd(SEG_UNIT(SegNo));      // unit to load the code from
        word SegBlock = MemRd(SEG_BLOCK(SegNo));    // start block; 0 if a data segment (then don't read anyting)
        word SegSize = MemRd(SEG_SIZE(SegNo));      // and length in bytes
        ...
        else
        {
            ...
            SegDict[SegNo].Seg = WordIndexed(Kp, -1);   // TODO: this is inconsistent with above. Is it correct?
        }
    }
...
void traceenterexitproc(size_t s, size_t p, bool entering)
{
#if 0
    if (ftrace == NULL)
        ftrace = fopen ("c:\\Users\\fseide\\AppData\\Local\\Packages\\c45d4f76-b1cf-4443-bcb4-ec12069e26fb_t3n3q47tzshd8\\LocalState\\Volumes\\tracemiller.txt", "wt");
    procidentity pr = { s, p };
    if (!ftrace)
        return;
    ...
    if (entering)
    {
        fprintf(ftrace, "CALL (%d, %d)\n", pr.s, pr.p);
        if (depth > _countof(callstack))
            sin(1.0);
        callstack[depth++] = pr;
    }
    else
    {
        auto top = callstack[depth-1];
        fprintf(ftrace, "EXIT (%d, %d)\n", pr.s, pr.p);
        if (top.s != pr.s || top.p != pr.p)
        {
            fprintf(ftrace, "?GOT (%d, %d)\n", top.s, top.p);
        }
        if (depth == 0)
            sin(1.0);
        depth--;
 ...
 *
 * Parameter are passed on the TOS stack (Sp) but then popped into the stack frame.
 *
 * If it's a segment procedure, the segment has already been loaded and ref-counted. (??)
 *
 * Procedures are identified by their segment and proc number.
 * static_link is something to do with the frame ??
 *
 * @returns
...
{
    traceenterexitproc(SegNumber(NewSeg), ProcNr, true);

    word NewJTab = Proc(NewSeg, ProcNr);                        // JTab entry describes the function (entry point, data size, param size)

    word DataSize = ProcDataSize(NewJTab);                      // size of all local variables
    word ParamSize = ProcParamSize(NewJTab);                    // size of all parameters

    word NewMp = WordIndexed(Kp, -(DataSize + ParamSize) / 2);  // open a new stack frame on the frame stack: local vars and params (more gets pushed below)

    if (!ProcNumber(NewJTab))                           // function is in assembly code
    {
    ...
    MoveLeft(NewMp, 0, Sp, 0, ParamSize);               // copy parameters into stack frame (this is a memcpy() like function)
    Sp = WordIndexed(Sp, ParamSize / 2);                // pop the parameters

    NewMp = WordIndexed(NewMp, -MS_FRAME_SIZE);         // expand the stack frame to add the actual call information (what makes it a 'frame')
    ...
    MemWr(WordIndexed(NewMp, MS_KP), Kp);               // previous frame stack (note: MS_KP = -1, this is outside)
    MemWr(WordIndexed(NewMp, MS_STAT), static_link);
    MemWr(WordIndexed(NewMp, MS_DYN), Mp);              // previous frame pointer
    MemWr(WordIndexed(NewMp, MS_JTAB), JTab);           // the calling function's description
    MemWr(WordIndexed(NewMp, MS_SEG), Seg);             // and its segment
    MemWr(WordIndexed(NewMp, MS_IPC), Ipc);             // return address
    MemWr(WordIndexed(NewMp, MS_SP), Sp);               // previous TOS stack

    Kp = WordIndexed(NewMp, -1); /* Small Hack :-( */   // Kp is not part of normal stack frame, special to Miller
    Mp = NewMp;
    Seg = NewSeg;
    JTab = NewJTab;
    MemWr(LASTMP, Mp);          // previous frame pointer
#ifdef SEG
    MemWr(SEG, Seg);
#endif
#ifdef JTAB
    ...
    IpcBase = ProcBase(JTab);   // IpcBase = first instruction of this function
    Ipc = 0;                    // and we execute from here
    Level++;
    StackCheck();
    return (0);
}
...
ret(byte n)
{
    ...
    traceenterexitproc(OldSegNo, ProcNumber(JTab), false);

    while (n > 0)                               // transfer function result from stack frame into TOS stack
    ...
    Ipc = MemRd(WordIndexed(OldMp, MS_IPC));    // restor the program counter
    MemWr(LASTMP, Mp);
#ifdef SEG
    MemWr(SEG, Seg);
#endif
    ...
    if (OldSegNo != SegNumber(Seg))             // returned from a segment procedure--unload its code
...
Debugger(void)
{
    ...
    if ((int) Level > (int) TraceLevel) // TODO: fix the sign issue
        return;
    TraceLevel = 0x7fff;

    DisasmP(Buffer, SegNumber(Seg), IpcBase, Ipc, JTab, Sp);
    _snprintf(prompt, sizeof(prompt), "s%d, p%d, %4d:      %s      > ",
...
Processor()
{
    ...
    // if caller sets this flag from a parallel thread, then return from this routine,
    // leaving the state intact so that we can resume
    while (!Irq) 
    {
        /* CheckCallStack(); */
        if (TraceFile)
...
#if 0
        {
            ...
            if (ftrace)
            {
                ...
                static int lineno = 0;
                static int prevsegid = -1;
                static int prevprocid = -1;
                static int previpc = -1;
    if (lineno == 36714)
        sin(1.0);
                int stackheight = 2 * (SP_TOP - Sp);
                int segid = SegNumber(Seg);
                int procid = ProcNumber(JTab);
                if (prevsegid != segid || prevprocid != procid || previpc != Ipc)             // is the same in case of retried TermRead
                    fprintf(ftrace, "%d\tS=%d\tP=%d\tIP=%d\top=%d\tH=%d\n",
                        ++lineno,
                        segid, procid,
                        Ipc, (word)MemRdByte(IpcBase, Ipc),
        ...
        CurrentIpc = Ipc;
        Opcode = (byte) FetchUB(); /* fetch next instruction */
        switch (Opcode)
        {
        ...
        case LOD:
            /* LOD LOaD */
            p1 = FetchUB();
            Push(MemRd(IntermediateAddr(FetchB(), (byte) p1)));
            break;

        case LDA:
            /* LDA LOad Addres */
            p1 = FetchUB();
            Push(IntermediateAddr(FetchB(), (byte) p1));
            break;

        case STR:
            /* STR StoRe */
            p1 = FetchUB();
            MemWr(IntermediateAddr(FetchB(), (byte) p1), Pop());
            break;
        ...
        case LDE:
            /* LDE LoaD External */
            p1 = FetchUB();
            Push(MemRd(ExternalAddr(FetchB(), (byte) p1)));
            break;

        case LAE:
            /* LAE Load Addres External */
            p1 = FetchUB();
            Push(ExternalAddr(FetchB(), (byte) p1));
            break;

        case STE:
            /* STE STore External */
            p1 = FetchUB();
            MemWr(ExternalAddr(FetchB(), (byte) p1), Pop());
            break;
        ...
        case SAS:
            ...
            else
            {
                ...
                MemWrByte(Dest, 1, (byte) w); /* containing char on stack         */
            }
            break;
        ...
        case CLP:                               // call local proc  --a nested procedure?
            call(Seg, (byte) FetchUB(), Mp);
            break;

        case CGP:                               // call global proc
            call(Seg, (byte) FetchUB(), Base);
            break;

        case CIP:
            p1 = FetchUB();
            call(Seg, (byte) p1, StaticLink(Seg, (byte) p1));
            break;

        case CBP:                               // call base proc; BaseMp = Mp after load_system, so this may access some globals?
            call(Seg, (byte) FetchUB(), BaseMp);
            break;

        case CXP:                               // call external proc (cross-segment)
            p1 = FetchUB();                     // segment
            p2 = FetchUB();                     // proc no
            if (p1) /* Not for Segment 0 */
                CspLoadSegment((byte) p1);      // load and push segment into the segment stack (or ref-count if already loaded)
            w = SegDict[p1].Seg;
            if (call(w, (byte) p2, StaticLink(w, (byte) p2)))
            {
                /*
        ...
        case CSP:
            ...
            case CSP_UREAD:
                {
                    ...
                    {
                        // BUGBUG: This only works for single reads;
                        // it will fail if a multi-byte read blocks in the middle.
                        // Then the read bytes will be discarded.
                        // recover the stack
                        Push(w1);
                        Push(w2);
                        Push(w3);
                        ...
                        Push(w5);
                        Push(w6);
                        Ipc = CurrentIpc;   // recover PC
                        Irq = 1;            // request interrupt ourselves
                    }
                }
                break;
...
#ifdef CSP_LDSEG
            case CSP_LDSEG:
                CspLoadSegment((byte) Pop());
                break;
#endif

#ifdef CSP_ULDSEG
            case CSP_ULDSEG:
                CspUnloadSegment((byte) Pop());
                break;
#endif
            ...
            case CSP_SIN:
                PushReal((float) sin(PopReal()));
                break;

            case CSP_COS:
                PushReal((float) cos(PopReal()));
                break;
            ...
            case CSP_SQRT:
                ...
                if (f < 0)
                    XeqError(XFPIERR);
                PushReal((float) sqrt(f));
                break;

            case CSP_ATAN:
                PushReal((float) atan(PopReal()));
                break;

            case CSP_EXP:
                PushReal((float) exp(PopReal()));
                break;
            ...
            case CSP_POT:
                {
                    float PwrOfTen[] = { 1e0f, 1e1f, 1e2f, 1e3f, 1e4f, 1e5f,
                        ...
                        1e36f, 1e37f, 1e38f//, 1e39f
                    };
                    int Value = PopInteger();
                    if ((Value < 0) || (Value >= sizeof(PwrOfTen) / sizeof(*PwrOfTen)))
            ...
            case CSP_MAV:
                if (MemRd(GDIRP))
                    ...
                    w = Kp - Np;
                Push(w / 2);    // BUGBUG: should only do this if not WORD_MEMORY mode, but then need to clip against MAXINT
                break;
        ...
        case XIT:
#if 1
            for (const char * s = "SYSTEM HALTED"; *s; s++)
                TermWrite(*s, 0);
            return;
#else
            XeqError(XHLTBPT);
...
// boot loader
// BlockNo = firstBlock of file to load
static void
load(word Unit, word BlockNo)
{
    int i;

    DiskRead(Unit, Np, 0, 512, BlockNo);    // read one header block (using Np (unallocated region) as temp area)
    if (MemRd(IORSLT))
        return;

    /* Create the Segment Dictionary */
    for (i = 0; i < 16; i++)            // read up to 16 segments
    {
        word CodeAddr = MemRd(WordIndexed(Np, 2 * i)) + BlockNo;    // this is information in the header block
        word CodeLeng = MemRd(WordIndexed(Np, 2 * i + 1));
        word SegInfo = MemRd(WordIndexed(Np, i + 0x80));

        assert(!(CodeLeng & 1));

        // code is read into code stack (base Kp)
        if (CodeAddr && CodeLeng)
        {
            int SegNo = SegInfo & 0xff;
            // try to understand both Apple disks (with seg mapping) and plain UCSD format (mappings are all zero)
            if (i > 0 && i < 15/*Apple split seg is also SegNo=0*/ && SegNo == 0)
            {
                SegNo = i;
                SegInfo = 0x100;    // a hack
            }
            if (SegInfo & 0x0f00)
            {
            ...
            if (SegNo == 0)     // segment 0 is put into the code stack and the segment table
            {
                ...
                else            // other segments are just pushed onto the code stack, without use count etc.
                {
#ifndef WORD_MEMORY
...
// load the SYSTEM.PASCAL file and set up p-code interpreter to call into it
// The file is searched for in all volumes, first on *root_unit_p, and *root_unit_p is updated with where it was found
static void
load_system(int *root_unit_p, const char *file_name)
{
    ...
    // get system file from specified root first, and if not present, search for it over all disks
    // file_name is SystemName which is "system.pascal"
    ...
    call(SegDict[0].Seg, 1, NIL);   // set execution to execute boot loader (note: call() does not execute it; it just sets the program counter and fixes up the stack)
    *root_unit_p = unit;
}
...
main(int argc, char **argv)
{
    ...
    {
        char * optarg = NULL;
        i = EOF;//getopt_long(argc, argv, "ab:Ddgn:t:T:w:r:f:xV", options, 0);
        if (i == EOF)
            break;
        switch (i)
        {
    ...
    root_unit = 4;      // try first on unit 4
    load_system(&root_unit, SystemName);    // this will set up p-code interpretation on boot code, and return the actual root_unit
    BaseMp = Mp;
    ...
    // TODO: this belongs into p_system, not p_cpu
    if (emulate_date)
    {
...
// TODO: This is currently lots of code dup from above.

static int             always_dump_core;
static int             Unit;
static int             BatchFd;
static const char      *SystemName;
static int             emulate_date;
static int             root_unit;
...
int/*bool*/ InitPCode(size_t numDisks, char diskModes[], const char * diskPaths[], const char * batchInput)
{
...
#ifndef WORD_MEMORY
    int appleCompat = 1;
    AppleCompatibility = appleCompat;
#endif

    if (batchInput)
    {
        if (strcmp(batchInput, "-") == 0)
            BatchFd = 0;
        else
            BatchFd = explain_open_or_die(batchInput, O_RDONLY, 0);
    }
    ...
    // mount disks
    for (size_t disk = 0; disk < numDisks; disk++)
    {
        char diskMode = diskModes[disk];
        const char * diskPath = diskPaths[disk];
        enum DiskMode Mode = ReadOnly;
        switch(diskMode)
        {
        ...
        if (DiskMount(Unit, diskPath, Mode) < 0)
            return 0;   // failed
        ...
        * the first disk.
        *
        * It is essential that the bytes sex it set before
        * the first of the initialization code that
        * establishes the contents of emulated memory.
        */
        if (disk == 0)
            byte_sex = disk_get_byte_sex(Unit);
        else if (byte_sex != disk_get_byte_sex(Unit))
            return 0;   // failed with inconsistent disks
    ...
    MemWr(LocalAddr(1), Syscom);        // pass SYSCOM record as a local variable
    ...
    // NOTE: it is jot clear why we cannot just jam the values in, and overwrite them from a second thread if they change
...
// interrupt ongoing RunPCodeUntilInterrupt()
// Call this from a different thread, obviously.
void InterruptPCode()
{
    Irq = 1;
}

// will return upon suspend flag being set
// Call again to continue if this returns 1.
