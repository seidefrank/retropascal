...
// This emulates native entry points.
// Each emulated function is recognized by its 'signature', which is the first bytes
// of the actual code. The hope is that these are unique.
// E.g. many signatures begin with 104 ($68), that is the PLA instruction.
// (TODO: is that actual 6502 code or some intermediate?)
// Some functions have a replacement library. For those, the functions are hacked in
// such that their "code" consists just of an ASCII string. Example: "applestu::random".
// The units share the same prefix of this string.
// The emulated function then reads out this "code" and performs string comparisons to
// know which function to execute.
// However, this does not seem to work with the original Apple disks.
...
static const byte TurtleSig[16] = { 'T', 'U', 'R', 'T', 'L', 'E', 'G', 'R', 'A', 'P', 'H', 'I', 'C', 'S', ':', ':' };
#endif
...
static const byte LongIntSig2[7] = { 'd', 'e', 'c', 'o', 'p', 's' };
static const byte transcen_sig[9] = { 't', 'r', 'a', 'n', 's', 'c', 'e', 'n', ':' };
...
transcen(word base)
{
    ...
    {
        PopReal();
        PushReal((float) log(PopReal()));
        return;
    }
    if (0 == strcmp(func, "log"))
    {
        PopReal();
        PushReal((float) log(PopReal() * M_LOG10E));
        return;
    }
    if (0 == strcmp(func, "sqrt"))
    {
...
static const byte applestu_sig[9] = { 'a', 'p', 'p', 'l', 'e', 's', 't', 'u', ':' };
...
static const signature Routines[] =
{
    ...
    // missing: paddle, button, ttlout, keypress, random, randomiz, note
    // faked modules
...
// signatures gathered
static const byte WSTRINGSig[] = {
 0x68, 0x85, 0x00, 0x68, 0x85, 0x01, 0x68, 0x29,
 0x0f, 0x85, 0x03, 0x68, 0x68, 0x85, 0x0b, 0x68,
 0x85, 0x0c, 0x68, 0x85, 0x09, 0x68, 0x85, 0x0a,
 0x68, 0x85, 0x0e, 0x68, 0x85, 0x0f, 0x68, 0x85,
 0x10, 0x68, 0x85, 0x11, 0x68, 0x85, 0x12, 0x68
};
static const byte MOVETOSig[] = {
 0xa0, 0x08, 0xb1, 0xfe, 0x99, 0x09, 0x00, 0xc8,
 0xc0, 0x0c, 0xd0, 0xf6, 0x68, 0x85, 0x00, 0x68,
 0x85, 0x01, 0x68, 0x85, 0x18, 0x68, 0x85, 0x19,
 0x68, 0x85, 0x16, 0x68, 0x85, 0x17, 0xa5, 0x01,
 0x48, 0xa5, 0x00, 0x48, 0xa0, 0x08, 0xb9, 0x0e
};
signature TriggerEntry = { MOVETOSig, sizeof(MOVETOSig), NULL };

void
ProcessNative(word JTab)
{
    word EntryPoint = ProcBase(JTab);
#if 0   // helper for gathering signatures --not really working
    // get signature
    char sig[200] = { 0 };
    for (size_t j = 0; j < _countof(sig) && j < 200; j++)
        sig[j] = MemRdByte(EntryPoint, j);
    const char function[80] = { "INITTURTLE" } ;
    word addr = Pop();
    const char * outpath = "C:/Users/Gloria Wang/AppData/Local/Packages/c45d4f76-b1cf-4443-bcb4-ec12069e26fb_t3n3q47tzshd8/LocalState";
    char fname[1000];
    sprintf(fname, "%s/%s.c", outpath, function);
    FILE * f = fopen (fname, "w");
    if (f)
    {
        fprintf(f, "static const byte %sSig[] = {\n", function);
        size_t nbytes = 40;
        for (size_t i = 0; i < nbytes; i++)
        {
            fprintf(f, " 0x%02x", (unsigned char) sig[i]);
            if (i < nbytes -1) fprintf(f, ",");
            if (i % 8 == 7 || i == nbytes-1)
                fprintf (f, "\n");
        }
        fprintf(f, "};\n");
